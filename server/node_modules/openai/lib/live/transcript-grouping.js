...
exports.TranscriptGrouping = void 0;
...
class TranscriptGrouping {
    ...
    standaloneAcknowledgment(fragment, previous) {
        return (fragment.endMs - (previous?.startMs ?? fragment.startMs) < this.options.backchannelMaxDurationMs &&
            this.acknowledgments.includes(normalizeAcknowledgment((previous?.text ?? '') + fragment.text)));
    }
    possibleAcknowledgment(fragment) {
        const text = normalizeAcknowledgment((this.buffered?.text ?? '') + fragment.text);
        return (fragment.endMs - (this.buffered?.startMs ?? fragment.startMs) < this.options.backchannelMaxDurationMs &&
            text.length > 0 &&
            this.acknowledgments.some((acknowledgment) => acknowledgment.startsWith(text)));
    }
}
exports.TranscriptGrouping = TranscriptGrouping;
//# sourceMappingURL=transcript-grouping.js.map
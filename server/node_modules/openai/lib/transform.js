...
function assignSchema(target, ...sources) {
    for (const source of sources) {
        if (Object.prototype.propertyIsEnumerable.call(source, '__proto__') && !(0, utils_1.hasOwn)(target, '__proto__')) {
...
function planPromotedRootAnyOfDefinitionRenames(root, branch) {
    ...
    for (const keyword of ['$defs', 'definitions']) {
        ...
        let aliasIndex = 0;
        for (const [name, definition] of Object.entries(branchDefinitions)) {
            if (!(0, utils_1.hasOwn)(rootDefinitions, name) || schemasEqual(rootDefinitions[name], definition)) {
                continue;
            }
...
function ensureStrictJsonSchema(jsonSchema, path, root) {
    ...
    if (hasObjectShape(jsonSchema)) {
        if (!(0, utils_1.hasOwn)(jsonSchema, 'additionalProperties')) {
    ...
    if (hasObjectShape(jsonSchema)) {
        for (const key of required) {
            if (!isObject(properties) || !(0, utils_1.hasOwn)(properties, key)) {
    ...
    if (Array.isArray(items)) {
        throw new TypeError(`Schema at \`${path.join('/') || '<root>'}\` uses tuple-form \`items\`, which cannot be represented in strict Structured Outputs.`);
    }
...
function resolvePointerPart(resolved, part) {
    if (Array.isArray(resolved)) {
        ...
        if (!(0, utils_1.hasOwn)(resolved, index)) {
            return undefined;
        }
        return resolved[index];
    }
    if (!isObject(resolved) || !(0, utils_1.hasOwn)(resolved, part)) {
        return undefined;
    }
    ...
    const rewriteRef = (ref) => {
        ...
        for (const [index, part] of originalParts.entries()) {
            ...
            if (part === 'anyOf' &&
                ...
                if (!(0, utils_1.hasOwn)(branches, branchIndex)) {
                    return ref;
                }
...
function preserveAllOfRefTargets(root, rootOnly = false) {
    ...
    for (const ref of refsToPreserve) {
        ...
        while ((0, utils_1.hasOwn)(definitions, alias)) {
...
function mergeObjectAllOf(jsonSchema, path, root, normalizing = new Set()) {
    ...
    const excludesRequiredProperty = closedProperties !== undefined && [...mergedRequired].some((key) => !closedProperties.has(key));
    ...
    for (const { key, propertySchema } of propertyEntries) {
        ...
        if ((0, utils_1.hasOwn)(mergedProperties, key) && !schemasEqual(mergedProperties[key], propertySchema)) {
            fail();
        }

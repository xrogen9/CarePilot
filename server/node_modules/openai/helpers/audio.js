...
function isResponse(stream) {
    ...
    const nodeReadable = 'pipe' in stream &&
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (isResponse(stream)) {
                ...
                source = node_stream_1.Readable.from(stream.stream());
            }
            else {
                source = stream;
            }
            ...
            (0, node_stream_1.pipeline)(source, ffplay.stdin, (error) => {
                if (error) {
                    ffplay.kill();
                    reject(error);
                    return;
...
function removeRecordingAbortListener(signal, listener) {
    try {
...
function nodejsRecordAudio({ signal, device, timeout } = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let rejectRecording = reject;
        const collectData = (chunk) => {
            if (!settled) {
                data.push(chunk);
            }
        };
        const stopRecording = () => {
            if (!settled && ffmpeg?.pid) {
                try {
                    wasStopped || (wasStopped = ffmpeg.kill('SIGTERM'));
                }
                catch (error) {
        ...
        try {
            ...
            ffmpeg = (0, node_child_process_1.spawn)('ffmpeg', [
                '-f',
                provider,
                '-i',
